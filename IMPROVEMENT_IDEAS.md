...
### 6.1 ROI点数ラベル（食品）
- **狙い**: 食品選ぶときにいちいち採点ではなく、コスパを俯瞰して感覚的に認知できるように
- **内容**: 食品にROI点数をつける（地域やスーパーで変わるけど大体の目安）
- **拡張案**: 過去に買ったものから学習してラベル更新
- **後回し理由**: 必要性不明、実装難易度高い

### 6.2 RecipeScreen（料理機能）
- 今は見せない
- **後回し理由**: 優先度低

### 6.3 PhotoAnalysisModal（写真解析）
- 今は見せない
- **後回し理由**: 精度問題、後で改善

### 6.4 AI機能パーソナライズ
- **内容**: ユーザーごとにAIの振る舞い・出力をカスタマイズ
- **例**: 説明の詳細度、トーン、言語など
- **後回し理由**: リリース後、テスト的に触ってもらってから検討

---

## 7. 栄養素計算の改善（優先度：高）

### 7.1 ButcherSelectとHomeScreenの目標値不一致を修正
- **問題**: ButcherSelect（食品選択画面）が固定値を使用、HomeScreenが動的値を使用
- **原因**:
  - ButcherSelect: `DEFAULT_CARNIVORE_TARGETS`（例: sodium 5000mg固定）
  - HomeScreen: `getCarnivoreTargets()`（例: 女性+妊娠中 = sodium 6500mg）
...
- **実装方針**:
  1. 現在の不足栄養素を計算
  2. 各食品が補える量を計算
  3. 上位3種の栄養素を多く含む食品に色付け
- **監視対象**: 実装後、UIの見やすさ・実用性を確認
- **補足**: 「今日は〇〇が足りません」メッセージは不要（ゲージで既にわかる）

### 7.3 複雑な計算式の実装（将来）
- **狙い**: あらゆる要素を考慮した高精度な栄養目標値計算
- **例**:
  - 睡眠時間 → ストレスレベル → マグネシウム必要量増加
  - 運動強度 → 代謝率 → タンパク質・脂質必要量増加
  - デバイス連携（Oura Ring, Whoop等）→ HRV/睡眠スコア → 電解質補正
- **実装場所**: `getCarnivoreTargets()` 関数内
- **技術要件**:
  - TypeScript型システムで可読性を保つ
  - 要素間の依存関係を明確化
  - ドキュメントで計算ロジックを図示
- **現在の状態**: 基本的な動的計算は実装済み（性別・年齢・活動量など）
- **将来拡張**:
  - ウェアラブルデバイス連携
  - 過去データからの学習
  - 季節・天候による補正
