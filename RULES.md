...
### 7.9. ガイドの書き方【チャットに全文・表形式】
- **ガイドはチャットに書く**。ファイルへの誘導（「〇〇.md を見て」）だけで終わらせない。手順・入力値は**その場の返答に全部出す**。
- **表形式**で書く。列は少なくとも **#（番号）・行動・場所** を入れる。場所の欄には「右下」「画面上部」「ダイアログ内」などと書く。
- **行動は毎回ナンバリング**（1. 2. 3. または表の # 列）。
- **コピペするものはコードブロック**（パス・パスワード・コマンドなど）。
- 拾い読みしやすく、簡潔に。「〇〇を入れる」だけにせず、**入れる値も同じ返答内に書く**。
