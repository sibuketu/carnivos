...
### 6.3b. タスク重複防止【Deduplication Protocol】
**同じタスクを何重にもやらないように、完了済みタスクを記録・確認する。**

- **記録**: タスク完了時に `AGENT_LOG.md` に「何をやったか」を具体的に記録する（ファイル名・キー名レベル）
- **確認**: 作業開始前に `AGENT_LOG.md` の直近ログを読み、同じタスクが既に完了していないか確認する
- **E2E以外でも**: i18n対応・Lint修正・リファクタリング等、繰り返し発生しやすいタスクは特に注意する
- **粒度**: 「i18nやった」ではなく「StreakCalendar, ArgumentCard, AppIconGenerator のi18n完了」のようにファイル単位で記録する
