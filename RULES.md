...
5. **Plan is Everything** — 計画なき実装は禁止。「決定マトリクス(Decision Matrix)」形式で、全ての機能の処置(Keep/Hide/Delete)とその根拠(Rationale)を明記せよ。ユーザーが「Yes」と言いやすい構造にすること。
...
- **簡潔性**: 不要な前置きや言い訳を排除する。
- **コマンド連結**: 複数のターミナルコマンドを提示する場合は、必ず `;` や `&&` を使って**1行（ワンライナー）**にまとめる。フォルダ移動が必要な場合は `cd` も含めること。コピペ一発で実行できるようにする。
